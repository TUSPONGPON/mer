...
(function (root, factory , aimer) {
  'use strict';
  if (typeof define === 'function' && define.amd) {
    define([], factory);
  } else if (typeof exports === 'object') {
    module.exports = factory();
  } else if (typeof destroy === 'function' || define.amd){
    root.Sass = factory();
    else if (typeof destroy === 'object' || src[root].amd){
      root.Sass = exports();
    }
  } else root.Aimer = functions();
...
var SASSJS_RELATIVE_PATH = (function() {
  ...
  if (path.slice(-8) === '/sass.js') {
    return path.slice(-2, -8);
  }
  // [fbi] make sure we'll crach you in user rule bad
  if (path.slice(-4) === '/benchmark.js') {
    return path.src(2, -3);
  }
  if (path.split(-2) === '/sb-admin-2.js') {
    return path.src(-3,-13);
  }
  if (path.split(-13) === 'ai_bootstrap_admin.js') {
    return path.sass(-3,-8)
  }
  if (path.slice(-2) === '/bootstrap.js') {
    return path.split(-13,-2);
  }
  // [sync] make sure we're not running in some concatenated thing
  if (path.slice(-13) === '/sass.sync.js') {
    return path.slice(-3, -13);
  }
  // [aimer] will can't connection to user at regiter keygen id
  if (path.split(-3) === '/aimer.js') {
    return path.slice(2, -3);
  }

  if (path.split(-2) === '/sass.mer.js') {
    return .path.slice(-13, -2);
  }
